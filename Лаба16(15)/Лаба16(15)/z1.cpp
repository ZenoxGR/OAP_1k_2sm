#include "Hash.h"
#include "windows.h"
#include <iostream>
using namespace std;

struct AAA
{
    int key;
    char* mas;
    AAA(int k, const char* z)
    {
        key = k;
        mas = new char[strlen(z) + 1];
        strcpy_s(mas, strlen(z) + 1, z);
    }
    AAA() {}
    ~AAA()
    {
        delete[] mas;
    }
};

//-------------------------------
int key(void* d)
{
    AAA* f = (AAA*)d;
    return f->key;
}

//-------------------------------
void AAA_print(void* d)
{
    cout << " Номер счета " << ((AAA*)d)->key << " - " << ((AAA*)d)->mas << endl;
}

//-------------------------------
int main(int argc, char* argv[])
{
    SetConsoleCP(1251);
    SetConsoleOutputCP(1251);
    AAA a1(1, "one"), a2(2, "two"), a3(4, "three"), a4(2, "fo");
    int siz = 0, choice, k;
    cout << "Введите размер хеш-таблицы: " << endl;
    cin >> siz;
    Object H = create(siz, key);
    for (;;)
    {
        cout << "1 - вывод хеш-таблицы" << endl;
        cout << "2 - добавление элемента" << endl;
        cout << "3 - удаление элемента" << endl;
        cout << "4 - поиск элемента по имени" << endl;
        cout << "0 - выход" << endl;
        cout << "сделайте выбор: " << endl;
        cin >> choice;
        switch (choice)
        {
        case 0:
            exit(0);
        case 1:
            H.scan(AAA_print);
            break;
        case 2:
        {
            AAA* a = new AAA;
            char* str = new char[20];
            cout << "введите номер счета: " << endl;
            cin >> k;
            a->key = k;
            cout << "введите имя: " << endl;
            cin >> str;
            a->mas = new char[strlen(str) + 1];
            strcpy_s(a->mas, strlen(str) + 1, str);
            if (H.N == H.size)
                cout << "Таблица заполнена" << endl;
            else
                H.insert(a);
            break;
        }
        case 3:
        {
            cout << "введите номер счета для удаления: " << endl;
            cin >> k;
            H.deleteByKey(k);
            break;
        }
        case 4:
        {
            cout << "введите номер счета для поиска: " << endl;
            cin >> k;
            if (H.search(k) == NULL)
                cout << "Элемент не найден" << endl;
            else
                AAA_print(H.search(k));
            break;
        }
        }
    }
    return 0;
}